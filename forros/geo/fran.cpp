...
// me dice si la recta a-b contiene a o
bool en_recta(pt o, pt a, pt b) { return det(a-o, b-o) == 0; }
// me dice si a y b son colineares con o al mismo tiempo Y están en la misma dirección respecto a o
...
vector<pt> right_hull(vector<pt> a) {
	...
	forr(i, 2, sz(a)) {
		...
		// while (j >= 2 && cw(a[j-2], a[j-1], a[i])) j--; // no borra colineales
		a[j++] = a[i];
	} a.resize(j); return a;
}
